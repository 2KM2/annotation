...
#define MAVLINK_MSG_ID_FORMATION_ACK_CRC 0
...
static inline uint16_t mavlink_msg_formation_ack_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint8_t cmd, uint8_t result, uint16_t id, uint8_t type, uint32_t token)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id,MAVLINK_MSG_ID_FORMATION_ACK_LEN,MAVLINK_MSG_ID_FORMATION_ACK_LEN, MAVLINK_MSG_ID_FORMATION_ACK_CRC);
}
						           ...
						           uint8_t cmd,uint8_t result,uint16_t id,uint8_t type,uint32_t token)
{
	...
	msg->msgid = MAVLINK_MSG_ID_FORMATION_ACK;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan,MAVLINK_MSG_ID_FORMATION_ACK_LEN,MAVLINK_MSG_ID_FORMATION_ACK_LEN, MAVLINK_MSG_ID_FORMATION_ACK_CRC);
}
...
{
	return mavlink_msg_formation_ack_pack(system_id, component_id, msg, formation_ack->cmd, formation_ack->result, formation_ack->id, formation_ack->type, formation_ack->token);
}
...
{
	return mavlink_msg_formation_ack_pack_chan(system_id, component_id, chan, msg, formation_ack->cmd, formation_ack->result, formation_ack->id, formation_ack->type, formation_ack->token);
}
...
static inline void mavlink_msg_formation_ack_send(mavlink_channel_t chan, uint8_t cmd, uint8_t result, uint16_t id, uint8_t type, uint32_t token)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[MAVLINK_MSG_ID_FORMATION_ACK_LEN];
	_mav_put_uint32_t(buf, 0, token);
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_FORMATION_ACK, buf, MAVLINK_MSG_ID_FORMATION_ACK_LEN, MAVLINK_MSG_ID_FORMATION_ACK_CRC);
#else
	mavlink_formation_ack_t packet;
	packet.token = token;
	packet.id = id;
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_FORMATION_ACK, (const char *)&packet, MAVLINK_MSG_ID_FORMATION_ACK_LEN, MAVLINK_MSG_ID_FORMATION_ACK_CRC);
#endif
}
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_formation_ack_send(chan, formation_ack->cmd, formation_ack->result, formation_ack->id, formation_ack->type, formation_ack->token);
#else
    _mav_finalize_message_chan_send_allid(chan, 1, 2, MAVLINK_MSG_ID_FORMATION_ACK, (const char *)formation_ack, MAVLINK_MSG_ID_FORMATION_ACK_LEN, MAVLINK_MSG_ID_FORMATION_ACK_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_formation_ack_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t cmd, uint8_t result, uint16_t id, uint8_t type, uint32_t token)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
