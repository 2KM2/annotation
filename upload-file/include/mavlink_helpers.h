...
MAVLINK_HELPER void crc_add(uint8_t* pBuffer, uint16_t length, uint8_t ext)
{
	uint16_t crc_val = 0, trans_val;
	
    crc_val = crc_calculate((const uint8_t*)pBuffer, length);
    crc_accumulate(ext, &crc_val); //extend CRC , 

    pBuffer[length] = (crc_val & 0xff);
    pBuffer[length+1] = ((crc_val >> 8) & 0xff);
}
MAVLINK_HELPER int MavlinkMsgPack(uint8_t* pack, uint8_t* data, int id, int len, int extcrc)
{
    int ret = 0;
	pack[0] = 0xfe; // mavlink_stx
	pack[1] = len;
	pack[2] = 0; // current_tx_seq
	pack[3] = 0; // sysid
	pack[4] = 0; // compid
	pack[5] = id;
	memcpy(&(pack[6]), data, len);
	crc_add(&(pack[1]), (len+5), extcrc);
	ret = len + 8;
    return ret;
}
